"""Tests for curfp.dsfr2 — symmetric rank-2 update in RFP format (double precision)."""
...
def test_dsfr2_vs_numpy(n, uplo, transr):
    """curfp.dsfr2: arf += alpha*(x*y^T + y*x^T) matches numpy (float64)."""
    ...
    curfp.dsfr2(arf, x_t, y_t, alpha=alpha, transr=transr, uplo=uplo)
    ...
    np.testing.assert_allclose(
        ...
        rtol=TOL,
        atol=TOL,
        err_msg=f"dsfr2 mismatch: transr={transr} uplo={uplo} n={n}",
    )
...
@pytest.mark.parametrize("uplo", UPLO_VALS)
@pytest.mark.parametrize("transr", TRANSR_VALS)
def test_dsfr2_zero_alpha(n, uplo, transr):
    ...
    x_t = torch.from_numpy(x_np).cuda()
    y_t = torch.from_numpy(y_np).cuda()
    curfp.dsfr2(arf, x_t, y_t, alpha=0.0, transr=transr, uplo=uplo)
...
def test_dsfr2_raw_api():
    """dsfr2_raw produces the same result as the high-level API."""
    n = 8
