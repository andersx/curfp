/*
 * curfpDstfttr — Copy triangular matrix from RFP format to standard full format
 *               (double precision)
 *
 * Double-precision copy of curfp_stfttr.cu: float -> double,
...
static __global__ void k_dstfttr(
...
extern "C"
curfpStatus_t curfpDstfttr(curfpHandle_t    handle,
                            ...
                            int              lda)
{
    ...
    if (n == 1) {
        CURFP_CHECK_CUDA(cudaMemcpy(A, arf, sizeof(double),
